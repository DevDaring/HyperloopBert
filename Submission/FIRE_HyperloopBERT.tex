...
unbiased model. The protocol therefore states three conditions in advance under
...
diverged later in training; Section~\ref{sec:stability} reports both events. The
% CAMERA-READY: restore the concrete artifact location for these snapshots
snapshots analysed here come from the run at $1.5 \times 10^{-4}$. Holding the learning
...
The contrasts, specified before analysis of the comparison point, appear in
Table~\ref{tab:confirm}. Both comparisons
...
\begin{table}[t]
\centering
\caption{Contrasts specified before analysis of the comparison point. Paired
permutation test over 1508 items with Holm correction.}
\label{tab:confirm}
\footnotesize
\begin{adjustbox}{max width=\columnwidth}
\begin{tabular}{lcccc}
