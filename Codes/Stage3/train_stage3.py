...
# CITATION: Zeitoun, A., Torroba-Hennigen, L., & Kim, Y. (2026). Hyperloop Transformers. arXiv:2604.21254.
...
def main():
    ...
    # We run standard configs, plus ablation
    runs_to_do = []
    ...
    for arch in cfg.ARCHITECTURES:
        for size in args.sizes:
            for seed in seeds_to_run:
                # Standard run
                runs_to_do.append({
                    'arch': arch, 'size': size, 'seed': seed, 'num_streams': cfg.DEFAULT_NUM_STREAMS
                })
                
                # Stream ablation for HyperloopBERT
                if arch == 'HyperloopBERT':
                    for ns in cfg.NUM_STREAMS_ABLATION:
                        if ns != cfg.DEFAULT_NUM_STREAMS:
                            runs_to_do.append({
                                'arch': arch, 'size': size, 'seed': seed, 'num_streams': ns
                            })
    ...
    for run_config in runs_to_do:
        ...
        num_streams = run_config['num_streams']
        
        run_id = f"{arch}_{size}_seed{seed}_streams{num_streams}"
        ...
        model = build_model(arch, size, num_streams=num_streams)
        ...
        optimizer = AdamW(model.parameters(), lr=cfg.LEARNING_RATE, 
                          betas=cfg.ADAMW_BETAS, eps=cfg.ADAMW_EPS, weight_decay=cfg.WEIGHT_DECAY)
        ...
        while tokens_processed < cfg.MAX_TOKENS:
            ...
            if (step + 1) % accum_steps == 0:
                ...
                if actual_step % cfg.VAL_EVERY_STEPS == 0:
                    ...
                    logger.info(f"Step {actual_step} | Val Loss: {val_loss:.4f} | PP: {p_perp:.2f} | TPS: {tps:.1f}")
